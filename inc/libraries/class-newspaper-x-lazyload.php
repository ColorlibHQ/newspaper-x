...
class Newspaper_X_LazyLoad {
	...
	function __construct() {
		...
		if ( $lazy ) {
			/*
			 * This used to POST every visitor's user agent to
			 * api.wordpress.org over plain HTTP on each page load, just to
			 * find Internet Explorer and strip srcset/sizes from images.
			 * Internet Explorer is gone, the request leaked the visitor's
			 * user agent to a third party, and on a cache miss it blocked
			 * rendering on an external round trip. It is all removed.
			 */
	...
	function enqueue_lazyload() {
		...
		wp_enqueue_script( 'newspaper-x-jquery_lazy_load', get_template_directory_uri() . '/assets/vendors/blazy/blazy.min.js', array( 'jquery' ), '1.9.1' );
	}
