...
class Widget_Newspaper_X_Banner extends WP_Widget {
	...
	public function form( $instance ) {
		wp_enqueue_media();
		wp_enqueue_style( 'newspaper_x_media_upload_css', get_template_directory_uri() . '/inc/assets/css/upload-media.css' );
		wp_enqueue_script( 'newspaper_x_media_upload_js', get_template_directory_uri() . '/inc/assets/js/upload-media.js', array( 'jquery' ) );
		...
		<p class="newspaper-x-media-control"
		   data-delegate-container="<?php echo esc_attr( $this->get_field_id( 'image_id' ) ) ?>">
			...
			<?php
			$image = wp_get_attachment_image_url( (int) $image_id, false, false );
			?>
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['image_id']  = (int) esc_html( $new_instance['image_id'] );
