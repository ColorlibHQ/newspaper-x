...
class NewspaperX_Breadcrumbs {
	...
	public function __construct() {
		...
		$defaults = array(
			'home_prefix'            => get_theme_mod( 'newspaperx_blog_breadcrumb_menu_prefix', __( '', 'newspaper-x' ) ),
			'separator'              => get_theme_mod( 'newspaperx_blog_breadcrumb_menu_separator', '/' ),
			'show_post_type_archive' => '1',
			'show_terms'             => get_theme_mod( 'newspaperx_blog_breadcrumb_menu_post_category', 1 ),
		...
		$args     = apply_filters( 'newspaperx_breadcrumbs_defaults', $defaults );
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = sprintf( '<div class="newspaperx-breadcrumbs">%s</div>', $this->html_markup );
	}
	...
	private function get_breadcrumb_prefix() {
		...
		if ( ! is_front_page() ) {
			...
			if ( $this->home_prefix ) {
				$prefix = sprintf( '<span class="newspaperx-breadcrumb-prefix">%s:</span>', esc_html( $this->home_prefix ) );
			}
		}
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = true, $microdata = true ) {
		...
		if ( $separator ) {
			$separator_markup = sprintf( '<span class="newspaperx-breadcrumb-sep">%s</span>', $this->separator );
		}
