...
class Bugle_Breadcrumbs {
	...
	public function __construct() {
		...
		$defaults = array(
			'home_prefix'            => get_theme_mod( 'bugle_blog_breadcrumb_menu_prefix', __( '', 'bugle' ) ),
			'separator'              => get_theme_mod( 'bugle_blog_breadcrumb_menu_separator', '/' ),
			'show_post_type_archive' => '1',
			'show_terms'             => get_theme_mod( 'bugle_blog_breadcrumb_menu_post_category', 1 ),
			'home_label'             => esc_html__( 'Home', 'bugle' ),
			'tag_archive_prefix'     => esc_html__( 'Tag:', 'bugle' ),
			'search_prefix'          => esc_html__( 'Search:', 'bugle' ),
			'error_prefix'           => esc_html__( '404 - Page not Found', 'bugle' ),
		);

		// Setup a filter for changeable variables and merge it with the defaults
		$args     = apply_filters( 'bugle_breadcrumbs_defaults', $defaults );
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = sprintf( '<div class="bugle-breadcrumbs">%s</div>', $this->html_markup );
	}
	...
	private function get_breadcrumb_prefix() {
		...
		if ( ! is_front_page() ) {
			...
			if ( $this->home_prefix ) {
				$prefix = sprintf( '<span class="bugle-breadcrumb-prefix">%s:</span>', esc_html( $this->home_prefix ) );
			}
		}
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = true, $microdata = true ) {
		...
		if ( $separator ) {
			$separator_markup = sprintf( '<span class="bugle-breadcrumb-sep">%s</span>', $this->separator );
		}
