...
/**
 * Class Bugle_Related_Posts_Output
 *
...
if ( ! function_exists( 'Bugle_CallRelatedPostsClass' ) ) {
	/**
	 *
	 ...
	 *
	 */
	function Bugle_CallRelatedPostsClass() {
		$display_related_blog_posts = get_theme_mod( 'bugle_related_posts_enabled', 'enabled' );

		if ( $display_related_blog_posts == 'enabled' ) {

			// instantiate the class & load everything else
			Bugle_Related_Posts_Output::getInstance();
		}
	}

	add_action( 'wp_loaded', 'Bugle_CallRelatedPostsClass' );
}


if ( ! class_exists( 'Bugle_Related_Posts_Output' ) ) {
	...
	class Bugle_Related_Posts_Output {
		...
		protected function __construct() {
			...
			if ( get_theme_mod( 'bugle_related_posts_enabled', 'enabled' ) == 'enabled' ) {
				...
				add_action( 'bugle_single_after_article', array( $this, 'output_related_posts' ), 2 );
			}
		...
		function output_related_posts() {
			...
			echo '<div class="bugle-related-posts">';

			// Number of posts to show / view
			$limit      = get_theme_mod( 'bugle_howmany_blog_posts', 3 );
			$show_title = get_theme_mod( 'bugle_enable_related_title_blog_posts', 1 );
			$show_date  = get_theme_mod( 'bugle_enable_related_date_blog_posts', 0 );
			$auto_play  = get_theme_mod( 'bugle_autoplay_blog_posts', 1 );
			 ...
			 * Heading
			 */
			echo '<div class="col-lg-11 col-sm-10 col-xs-12 bugle-related-posts-title">';
			echo '<h3><span>' . __( 'Related posts ', 'bugle' ) . '</span></h3>';
			echo '</div>';

			/*
			 * Arrows
			 */
			echo '<div class="bugle-carousel-navigation hidden-xs text-right">';
			echo '<ul class="bugle-carousel-arrows clearfix">';
			echo '<li><a href="#" class="bugle-owl-prev fa fa-angle-left"></a></li>';
			echo '<li><a href="#" class="bugle-owl-next fa fa-angle-right"></a></li>';
			echo '</ul>';
			echo '</div>';
			echo '</div><!--/.row-->';
			...
			while ( $related_posts->have_posts() ) {
				...
				if ( has_post_thumbnail( $related_posts->post->ID ) ) {
					$image = get_the_post_thumbnail( $related_posts->post->ID, 'bugle-recent-post-big' );
				}
				echo '<a href="' . esc_url( get_the_permalink() ) . '">' . $image . '</a>';

				if ( $show_title ) {
					echo '<div class="bugle-related-post-title">';
				...
				if ( $show_date ) {
					...
					echo '<div class="bugle-related-posts-date">';
