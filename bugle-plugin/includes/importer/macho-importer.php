	...
	class MT_Theme_Importer {
		...
		public function intro_html() {
			...
			<div class="about-text">
				<?php echo __( MT_THEME_NAME . " is now installed and ready to use!  Get ready to build something amazing.", "ensign" ); ?>
			</div>
			...
			if ( ! empty( $this->imported_demos ) ) { ?>
				...
				<p><?php _e( 'Demo already imported', 'bugle' ); ?></p>
				</div><?php
				//return;
				?>
		...
		public function demo_installer() {
			...
			if ( ! empty( $this->imported_demos ) ) {
				...
				$button_text = __( 'Import Again', 'bugle' );
			...
			<div class="macho-thanks">
				<hr/>
				<p class="description"><?php echo __( "Thank you for choosing ", "ensign" ) . ucfirst( MT_THEME_NAME ) . __( "We are fully dedicated to crafting the perfect experience for you.", 'bugle' ); ?></p>
			</div>
			</div>
			</div>
			<?php
		...
		public function set_demo_data( $file ) {
			...
			if ( $importer_error ) {
				...
				if ( ! is_file( $file ) ) {
					...
					set_theme_mod( 'bugle_recent_posts_category', array( '4', '7' ) );
		...
		function process_widget_import_file( $file ) {
			...
			// File exists?
			if ( ! file_exists( $file ) ) {
				wp_die( __( 'Widget Import file could not be found. Please try again.', 'bugle' ), '', array(
					'back_link' => true
				) );
			}
		...
		public function import_widgets( $data ) {
			...
			foreach ( $data as $sidebar_id => $widgets ) {
				...
				if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
					...
					$sidebar_message      = __( 'Sidebar does not exist in theme (using Inactive)', 'bugle' );
				}
				...
				foreach ( $widgets as $widget_instance_id => $widget ) {
					...
					if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
						...
						$widget_message      = __( 'Site does not support widget', 'bugle' ); // explain why widget not imported
					}
					...
					if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
						...
						foreach ( $single_widget_instances as $check_id => $check_widget ) {
							...
							if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
								...
								$widget_message      = __( 'Widget already exists', 'bugle' ); // explain why widget not imported
					...
					if ( ! $fail ) {
						...
						if ( $sidebar_available ) {
							...
							$widget_message      = __( 'Imported', 'bugle' );
						} else {
							$widget_message_type = 'warning';
							$widget_message      = __( 'Imported to Inactive', 'bugle' );
						}
					...
					if ( is_object( $widget ) ) {
						$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title'] = isset( $widget->title ) ? $widget->title : __( 'No Title', 'bugle' ); // show "No Title" if widget instance is untitled
					}
