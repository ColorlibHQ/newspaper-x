...
class Widget_Bugle_Banner extends WP_Widget {
	/**
	 * @internal
	 */
	public function __construct() {
		parent::__construct(
			'Bugle_Banner', // Base ID
			__( 'Bugle Banner', 'bugle' ), // Name
			array( 'description' => __( 'Bugle Banners', 'bugle' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( empty( $instance ) ) {
			$instance = array(
				'title'        => esc_attr__( 'Bugle Banners', 'bugle' ),
				'show_title'   => 'no',
				'image'        => '',
				'image_url'    => '',
				'banner_type'  => 'image',
		...
		$before_widget = str_replace( 'class="', 'class="bugle-' . $params['banner_type'][0] . ' ', $before_widget );
		echo $before_widget;
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'banner_type' ); ?>"><?php _e( 'Banner Type', 'bugle' );
				?>:</label>
