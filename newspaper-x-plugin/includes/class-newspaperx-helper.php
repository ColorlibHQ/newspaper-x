...
class NewspaperX_Helper {
	...
	public static function get_post_meta( $id ) {
		...
		$html .= '<li class="post-comments">' . $comments->approved . ' ' . __('Comments', 'newspaper-x') . '</li>';
		$html .= '<li class="post-category"><a href="' . esc_url( get_category_link( $cat[0] ) ) . '">' . get_the_category_by_ID( $cat[0] ) .'</a></li>';
		$html .= '</ul>';
