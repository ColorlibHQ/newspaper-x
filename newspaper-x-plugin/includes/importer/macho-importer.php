...
if ( ! defined( 'MT_THEME_NAME' ) ) {
	define( 'MT_THEME_NAME', 'Newspaper X' );
}
// Don't duplicate me!
if ( ! class_exists( 'MT_Theme_Importer' ) ) {

	class MT_Theme_Importer {
		/**
		...
		public function __construct() {
			...
			$this->imported_demos = get_option( 'mt_imported_demo' );
		...
		public function after_wp_importer() {
			...
			do_action( 'mt_importer_after_content_import' );

			update_option( 'mt_imported_demo', $this->flag_as_imported );

		}

		public function intro_html() {
			?>
			<div class="wrap about-wrap macho-wrap">

			<div class="about-text">
				<?php echo __( MT_THEME_NAME . " is now installed and ready to use!  Get ready to build something amazing.", "newspaperx" ); ?>
			</div>

			<div class="macho-registration-steps">
			<div
				style="background-color: #FFF; margin:10px 0;padding: 5px 10px;color: #333;border: 1px solid #EEE; clear:both; line-height:18px;">
				<p class="tie_message_hint">Importing demo data (post, pages, images, theme settings, ...) is the
					easiest way to setup your theme. It will
					allow you to quickly edit everything instead of creating content from scratch. When you import the
					data following things will happen:</p>

				<ul style="padding-left: 20px;list-style-position: inside;list-style-type: square;}">
					<li>Default posts, pages and comments will be deleted.</li>
					<li>The following WordPress settings will be modified: Permalinks -> will be set to %postname%;
						Reading Options -> Homepage / Posts page will be set to static pages created during the import
						process.
					</li>
					<li>Posts, pages, some images, some widgets and menus will get imported .</li>
					<li>Images will be downloaded from our server, these images are copyrighted and are for demo use
						only .
					</li>
			...
			if ( ! empty( $this->imported_demos ) ) { ?>
				...
				<div
					style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
				<p><?php _e( 'Demo already imported', 'newspaper-x' ); ?></p>
				</div><?php
				//return;
				?>
		...
		public function demo_installer() {
			...
			if ( ! empty( $this->imported_demos ) ) {
				...
				$button_text = __( 'Import Again', 'newspaper-x' );
			...
			<div class="mt-importer-wrap" data-demo-id="1"
			     data-nonce="<?php echo wp_create_nonce( 'mt-demo-code' ); ?>">

				<form method="post">
					<?php $this->intro_html(); ?>
					<input type="hidden" name="demononce" value="<?php echo wp_create_nonce( 'mt-demo-code' ); ?>"/>
					<input name="reset" class="panel-save button-primary mt-import-start" type="submit"
					       value="<?php echo $button_text; ?>"/>
					<input type="hidden" name="action" value="demo-data"/>

					<br/>
					<br/>
					<div class="mt-importer-message clear">
						<?php
						if ( 'demo-data' == $action && check_admin_referer( 'mt-demo-code', 'demononce' ) ) {
			...
			<div class="macho-thanks">
				<hr/>
				<p class="description"><?php echo __( "Thank you for choosing ", "newspaperx" ) . ucfirst( MT_THEME_NAME ) . __( "We are fully dedicated to crafting the perfect experience for you.", 'newspaper-x' ); ?></p>
			</div>
			</div>
			</div>
			<?php
		 ...
		 * @return null
		 */
		public function process_imports( $content = true, $options = true, $menus = true, $widgets = true, $reading_options = true ) {
		...
		public function set_demo_data( $file ) {
			...
			if ( $importer_error ) {
				...
				if ( ! is_file( $file ) ) {
					...
					set_theme_mod( 'newspaperx_recent_posts_category', array( '4', '7' ) );
		...
		public function set_demo_menus() {
			...
			foreach ( $menus as $name => $loc ) {
				$menu_exists = wp_get_nav_menu_object( $name );

				if ( ! has_nav_menu( $loc ) ) {
					$locations         = get_theme_mod( 'nav_menu_locations' );
					$locations[ $loc ] = $menu_exists->term_id;
		...
		function process_widget_import_file( $file ) {
			...
			// File exists?
			if ( ! file_exists( $file ) ) {
				wp_die( __( 'Widget Import file could not be found. Please try again.', 'newspaper-x' ), '', array(
					'back_link' => true
				) );
			}

			// Get file contents and decode
			$response = wp_remote_get( get_template_directory_uri() . '/demo-content/' . $this->widgets_file_name, array( 'timeout' => 15 ) );
		...
		public function import_widgets( $data ) {
			...
			foreach ( $data as $sidebar_id => $widgets ) {
				...
				if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
					...
					$sidebar_message      = __( 'Sidebar does not exist in theme (using Inactive)', 'newspaper-x' );
				}
				...
				foreach ( $widgets as $widget_instance_id => $widget ) {
					...
					if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
						...
						$widget_message      = __( 'Site does not support widget', 'newspaper-x' ); // explain why widget not imported
					}
					...
					if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
						...
						foreach ( $single_widget_instances as $check_id => $check_widget ) {
							...
							if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
								...
								$widget_message      = __( 'Widget already exists', 'newspaper-x' ); // explain why widget not imported
					...
					if ( ! $fail ) {
						...
						if ( $sidebar_available ) {
							...
							$widget_message      = __( 'Imported', 'newspaper-x' );
						} else {
							$widget_message_type = 'warning';
							$widget_message      = __( 'Imported to Inactive', 'newspaper-x' );
						}
					...
					if ( is_object( $widget ) ) {
						$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title'] = isset( $widget->title ) ? $widget->title : __( 'No Title', 'newspaper-x' ); // show "No Title" if widget instance is untitled
					}
