...
class Widget_NewspaperX_Banner extends WP_Widget {
	/**
	 * @internal
	 */
	public function __construct() {
		parent::__construct(
			'NewspaperX_Banner', // Base ID
			__( 'Newspaper X Banner', 'newspaper-x' ), // Name
			array( 'description' => __( 'Newspaper X Banners', 'newspaper-x' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( empty( $instance ) ) {
			$instance = array(
				'title'        => esc_attr__( 'Newspaper X Banners', 'newspaper-x' ),
				'show_title'   => 'no',
				'image'        => '',
				'image_url'    => '',
				'banner_type'  => 'image',
		...
		$before_widget = str_replace( 'class="', 'class="newspaperx-' . $params['banner_type'][0] . ' ', $before_widget );
		echo $before_widget;
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'banner_type' ); ?>"><?php _e( 'Banner Type', 'newspaper-x' );
				?>:</label>
			<select name="<?php echo $this->get_field_name( 'banner_type' ); ?>[]"
				...
				<option value="image" <?php echo ( $banner_type[0] == 'image' ) ? 'selected' : '' ?>>
					<?php echo __( 'Image', 'newspaper-x' ) ?>
				</option>
				<option value="adsense" <?php echo ( $banner_type[0] == 'adsense' ) ? 'selected' : '' ?>>
					<?php echo __( 'Adsense', 'newspaper-x' ) ?>
				</option>
			</select>
		</p>

		<p>
			<label for="<?php echo $this->get_field_name( 'image' ); ?>"><?php _e( 'Image:', 'newspaper-x'  ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'adsense_code' ); ?>"><?php _e( 'Adsense Code:' , 'newspaper-x' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'image_url' ); ?>"><?php _e( 'Image URL:', 'newspaper-x' ); ?></label>
